...
class Settings:
    ...
    vector_store: str = os.getenv("VECTOR_STORE", "mock").lower()
    ...
    rag_candidate_count: int = int(os.getenv("RAG_CANDIDATE_COUNT", "12"))
    rag_mmr_enabled: bool = _env_bool("RAG_MMR_ENABLED", "true")
    rag_mmr_lambda: float = float(os.getenv("RAG_MMR_LAMBDA", "0.7"))
    rag_reranker_enabled: bool = _env_bool("RAG_RERANKER_ENABLED", "true")
    rag_reranker_provider: str = os.getenv("RAG_RERANKER_PROVIDER", "mock").lower()
    rag_reranker_candidate_count: int = int(os.getenv("RAG_RERANKER_CANDIDATE_COUNT", "6"))
    ...
    bge_embedding_model_name: str = os.getenv("BGE_EMBEDDING_MODEL_NAME", "BAAI/bge-large-zh-v1.5")
    bge_embedding_device: str = os.getenv("BGE_EMBEDDING_DEVICE", "")
    bge_normalize_embeddings: bool = _env_bool("BGE_NORMALIZE_EMBEDDINGS", "true")
    bge_reranker_model_name: str = os.getenv("BGE_RERANKER_MODEL_NAME", "BAAI/bge-reranker-large")
    reranker_model_name: str = os.getenv("RERANKER_MODEL_NAME", "bge-reranker-large")
    ...
    milvus_collection: str = os.getenv("MILVUS_COLLECTION", os.getenv("CHROMA_COLLECTION", "customer_service_knowledge"))
    milvus_timeout_seconds: float = float(os.getenv("MILVUS_TIMEOUT_SECONDS", "3"))
    ...
    business_service_retry_attempts: int = int(os.getenv("BUSINESS_SERVICE_RETRY_ATTEMPTS", "2"))
    business_service_retry_backoff_ms: int = int(os.getenv("BUSINESS_SERVICE_RETRY_BACKOFF_MS", "50"))
    business_service_max_connections: int = int(os.getenv("BUSINESS_SERVICE_MAX_CONNECTIONS", "20"))
    business_service_max_keepalive_connections: int = int(
        os.getenv("BUSINESS_SERVICE_MAX_KEEPALIVE_CONNECTIONS", "10")
    )
    business_service_circuit_breaker_enabled: bool = _env_bool("BUSINESS_SERVICE_CIRCUIT_BREAKER_ENABLED", "true")
    business_service_circuit_failure_threshold: int = int(
        os.getenv("BUSINESS_SERVICE_CIRCUIT_FAILURE_THRESHOLD", "3")
    )
    business_service_circuit_reset_seconds: float = float(
        os.getenv("BUSINESS_SERVICE_CIRCUIT_RESET_SECONDS", "30")
    )
    readiness_timeout_seconds: float = float(os.getenv("READINESS_TIMEOUT_SECONDS", "1.5"))
    metrics_lite_enabled: bool = _env_bool("METRICS_LITE_ENABLED", "true")
    metrics_lite_window_size: int = int(os.getenv("METRICS_LITE_WINDOW_SIZE", "200"))
    rag_cache_enabled: bool = _env_bool("RAG_CACHE_ENABLED", "true")
    rag_cache_ttl_seconds: int = int(os.getenv("RAG_CACHE_TTL_SECONDS", "300"))
    rag_cache_max_size: int = int(os.getenv("RAG_CACHE_MAX_SIZE", "128"))
