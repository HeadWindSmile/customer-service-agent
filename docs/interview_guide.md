...
7. 可观测性：每轮都有 trace、span、event、tool_calls、sources 和 safety_result。
...
trace 里能看到：intent、slots、RAG sources、LLM usage、tool_calls、安全结果、事件投递结果和耗时。eval 数据集会检查 intent、关键词、sources、tool 和 safety 结果。
...
3. `/health`、`/ready` 和 `metrics-lite`。
4. Docker Compose 启动 AI 服务、mock 业务服务和 Redis。
