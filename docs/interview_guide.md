...
这个项目是一个企业级 AI 客服问答系统 Demo，不是普通 ChatBot。它模拟在原有 Spring Boot 主业务系统旁边新增 Python/FastAPI AI 服务层，让用户问题经过安全、记忆、意图识别、Router 分发后，知识类问题走 RAG，套餐、账单、工单、Offer、Order 等业务问题走工具调用。同时系统有 RBAC、审计、事件、trace、Prometheus-compatible `/metrics`、eval 和本地 load report，重点展示 Agent 工程化能力。

## 2 分钟项目介绍

项目背景是企业已有业务系统，AI 服务不能直接查业务库，也不能让大模型随意决定业务动作。所以我把系统拆成几个边界：API 层只接收请求；`CustomerAgent` 做主编排；`IntentClassifier` 负责规则 + LLM 结构化意图识别；`CustomerRouter` 根据 intent 分发；RAG 层负责知识库检索和 sources；Tools 层通过 `BusinessClient` 调用模拟业务系统 API。

为了让它更像企业项目，我还加了多轮 memory、RBAC、audit、安全检测、事件和 trace。比如客服代用户查账单或订单必须带 `target_user_id`，工具调用前会检查权限，敏感操作会写审计日志。每次请求会生成 `trace_id`，可以回放 RAG、LLM、工具、安全、事件投递和 latency breakdown。

当前 Demo 默认用 mock/fallback，保证本地没有真实 LLM、Milvus、BGE、Reranker、Redis、RocketMQ、数据库或外部监控平台也能启动。生产环境可以把这些边界替换为真实基础设施，但当前仓库不夸大这些能力。
...
3. 分发：FAQ、账单解释、故障排查走 RAG；套餐、账单、工单、Offer、Order 走工具。
...
8. 评测与报告：eval report 验证效果口径，load report 验证本地链路和报告格式。
9. 部署与兜底：本地默认 mock/fallback；Docker Compose 可启动 AI 服务、mock 业务服务和 Redis。

## 如何解释生产项目与当前仓库差异

推荐口径：

> 简历里的项目是生产项目，真实接入了业务系统和外部基础设施；当前仓库是脱敏后按阶段复现的版本，已经实现了 AI 服务层、RAG、LCEL、Router、工具调用、RBAC、安全、事件、trace、Prometheus-compatible `/metrics`、eval 和本地性能报告等核心架构。第 18 阶段已经把 README、简历映射、演示脚本、评测报告、压测报告和面试讲解口径整理成最终交付闭环。

回答时要把三类内容分开：

1. 生产项目真实能力：可以讲真实线上架构、效果指标和业务价值。
2. 当前仓库已实现能力：可以现场运行，用 curl、trace、eval、load report 和 pytest 证明。
3. 后续真实接入路线：说明当前仍是 mock/fallback/placeholder 的地方，以及如何替换为真实服务。

不要把生产指标说成本地仓库自动跑出的结果，也不要把当前 `mock_business_service` 说成真实 Spring Boot 服务。更准确的讲法是：当前仓库已经复现了架构边界，后续可以逐步替换为真实外部系统，同时保留本地 fallback。
...
4. trace 会记录 source_count、doc_ids、scores、cache_hit、MMR 和 reranker 相关配置。
...
`/metrics` 返回 Prometheus 0.0.4 文本格式，便于后续被 Prometheus 抓取。当前实现仍是单进程内存聚合，不默认部署 Prometheus、Grafana 或 OpenTelemetry Collector，也不提供多进程指标聚合、长期存储或告警规则。

## trace/eval 怎么讲

trace 用于单次请求复盘，eval 用于批量质量评估。

trace 里能看到：intent、slots、RAG sources、LLM usage、tool_calls、安全结果、事件投递结果和 latency breakdown。eval 数据集会检查 intent、关键词、sources、tool、安全动作、疑似幻觉、延迟和估算 Token/成本。

## load report 怎么讲

`scripts/simple_load_test.py` 可以生成 JSON/Markdown 本地性能报告，字段包括 avg、p50、p95、max、success_rate、error_rate、intent 分布和 status_code 分布。它只用于本地小流量验证，不代表生产级高并发、线上 SLA 或容量承诺。
...
### `/metrics` 是否代表已经接入 Prometheus/Grafana？

不代表。当前只是暴露 Prometheus-compatible 文本指标，后续可以被 Prometheus 抓取；完整监控平台仍需要部署 Prometheus、Grafana、OTel Collector、告警规则和长期存储。

### eval 报告里的 1.0 准确率是不是生产指标？

不是。它只代表当前本地 Demo 小样本评测集结果。生产指标需要更大标注集、真实模型、线上日志、人工质检和持续评测流水线。
